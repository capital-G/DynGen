 ...
 *  (id/code) and also another linked list (DynGen*) which stores all the
 * running DynGen instances with the associated code, which allows us to update
...
public:
     ...
     */
    static void dyngenAddFileCallback(World* inWorld, void* inUserData, struct sc_msg_iter* args, void* replyAddr);
...
private:
    ...
    static void rtCleanup(World* inWorld, NewDynGenLibraryEntry* newLibraryEntry, int numRtParameters);

    /*! @brief common function for loadScriptToDynGenLibrary() and
     * loadFileToDynGenLibrary() which creates and initializes the actual
     * DynGenScript instance.
     */
